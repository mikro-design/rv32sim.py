...
ee_s16 calc_func(ee_s16 *pdata, core_results *res)
{
	...
	if (optype) /* if cached, use cache */
		...
		ee_s16 flag = data &
			      0x7; /* bits 0-2 is type of function to perform */
		ee_s16 dtype =
			((data >> 3) &
			 0xf); /* bits 3-6 is specific data for the operation */
		dtype |=
			dtype
			<< 4; /* replicate the lower 4 bits to get an 8b value */
		switch (flag) {
		case 0:
			if (dtype <
			    0x22) /* set min period for bit corruption */
				dtype = 0x22;
		...
		retval &= 0x007f;
		*pdata = (data & 0xff00) | 0x0080 |
			 retval; /* cache the result */
		return retval;
	}
}
...
ee_u16 core_bench_list(core_results *res, ee_s16 finder_idx)
{
	...
	/* CRC data content of list from location of index N forward, and then
	 * undo remove */
	...
	/* sort the list by index, in effect returning the list to original
	 * state */
...
list_head *core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed)
{
	...
	ee_u32 size = (blksize / per_item) -
		      2; /* to accommodate systems with 64b pointers, and make
	...
	for (i = 0; i < size; i++) {
		...
		ee_u16 dat = (datpat << 3) |
			     (i & 0x7); /* alternate between algorithms */
		info.data16 = (dat << 8) |
			      dat; /* fill the data with actual data and
	...
	while (finder->next != NULL) {
		if (i < size / 5) /* first 20% of the list in order */
			...
			ee_u16 pat =
				(ee_u16)(i++ ^
					 seed); /* get a pseudo random number */
			finder->info->idx =
				0x3fff &
				(((i & 0x07) << 8) |
		...
		while (p) {
			...
			while (psize > 0 || (qsize > 0 && q)) {
				...
				if (psize == 0) {
					...
					/* First element of q is lower; e must
					 * come from q. */
					e = q;
					q = q->next;
					qsize--;
