...
main(int argc, char *argv[])
{
	...
	if (results[0].execs ==
	    0) { /* if not supplied, execute all algorithms */
...
#error "Please define a way to initialize a memory block."
	...
	if (results[0].iterations == 0) {
		...
		if (divisor == 0) /* some machines cast float to int as 0 since
				     this conversion is not defined by ANSI, but
				     we know at least one second passed */
			divisor = 1;
	...
	if (known_id >= 0) {
		for (i = 0; i < default_num_contexts; i++) {
			...
			if ((results[i].execs & ID_LIST) &&
				...
				ee_printf("[%u]ERROR! list crc 0x%04x - should "
					  "be 0x%04x\n",
					  i, results[i].crclist,
			...
			if ((results[i].execs & ID_MATRIX) &&
				...
				ee_printf("[%u]ERROR! matrix crc 0x%04x - "
					  "should be 0x%04x\n",
			...
			if ((results[i].execs & ID_STATE) &&
				...
				ee_printf("[%u]ERROR! state crc 0x%04x - "
					  "should be 0x%04x\n",
	...
	if (total_errors == 0) {
		ee_printf("Correct operation validated. See README.md for run "
			  "and reporting "
			  "rules.\n");
#if HAS_FLOAT
		if (known_id == 3) {
